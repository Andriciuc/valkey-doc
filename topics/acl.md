...
Although `SYNC` is deprecated, it is still used in dual-channel replication when the RDB channel requests a full resync. `PSYNC` is the preferred command for other replication scenarios.
